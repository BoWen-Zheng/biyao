		...
		$("input").blur(function(){
			if($(this).is("#name")){
				if(this.value==""){
					$("#name+span").remove();
					var str="用户名不能为空";
					$(this).after('<span class="text-danger">'+str+'</span>');
				}else{
					$("#name+span").remove();
					var str="输入正确";
					$(this).after('<span class="text-success">'+str+'</span>');
				}
			}
			if($(this).is("#password")){
				if(this.value==""){
			...
			$("button").click(function(){
				if($('#name+span').attr('class')=='text-success'&&$('#password+span').attr('class')=='text-success'){
				$("form").submit();
				}
			});
					  ...
					  <div class="panel-body">
						...
						<div class="col-xs-12">
							<form class="form-horizontal" action="login" method="POST">
								...
								<div class="col-xs-offset-10"><button type="button" class="btn btn-info">登录</button></div>
								</div>
							</form>	
						</div>
					</div>
